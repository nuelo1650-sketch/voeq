...
const PREFS_UI: Array<{ key: keyof Prefs; label: string; desc: string }> = [
  { key: 'notifyNewListings', label: 'New listings from followed vendors', desc: 'Get notified when vendors you follow post something new.' },
  { key: 'notifyNewReviews', label: 'New reviews on your wishlist', desc: 'Know when items you saved get new reviews.' },
...
export function ShopperOnboarding() {
  ...
  const savePrefs = async () => {
    ...
    try {
      ...
      setError('Could not save preferences. Check your connection and try again.');
      setSaving(false);
    }
  };
        ...
        <div className="mt-8 rounded-2xl border border-cream-300 bg-cream-50 p-6 dark:border-forest-700 dark:bg-forest-800 dark:border-cream-100">
          ...
          {step === 2 && (
            <div className="space-y-4">
              ...
              {!hasCampus && (
                <Button onClick={() => router.push('/select-campus')}>Select campus</Button>
              )}
              <div className="flex justify-between">
                <Button variant="ghost" onClick={() => setStep(1)}>Back</Button>
                <Button disabled={!hasCampus} onClick={() => setStep(3)}>Continue</Button>
              </div>
            </div>
          )}
          ...
          {step === 3 && (
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                {PREFS_UI.map((p) => (
                  <label key={p.key} className="flex cursor-pointer items-start gap-3 rounded-xl border border-cream-200 p-3 dark:border-forest-700">
                    <input
                      type="checkbox"
                      checked={prefs[p.key]}
              ...
              <div className="flex justify-between">
                ...
                <Button onClick={savePrefs} isLoading={saving}>Finish</Button>
              </div>
            </div>
          )}
        </div>
