...
import { ArrowRightIcon, StarIcon, CheckIcon, WhatsAppIcon, MapIcon } from '@/components/icons';
import {
  FoodIcon,
  TechIcon,
  FashionIcon,
...
const TRUST_PILLARS = [
  { icon: CheckIcon, title: 'Verified campus vendors' },
  { icon: WhatsAppIcon, title: 'Direct WhatsApp, no middleman' },
  { icon: StarIcon, title: 'Ratings & reviews' },
  { icon: MapIcon, title: 'Built for Nigerian students' },
];
            ...
            <div className="grid grid-cols-2 gap-8 sm:grid-cols-4">
              {TRUST_PILLARS.map((p) => (
                <div key={p.title} className="flex flex-col items-center text-center">
                  ...
                  <span className="text-sm font-medium text-forest-900 dark:text-cream-100">{p.title}</span>
                </div>
              ))}
            </div>
          </Container>
