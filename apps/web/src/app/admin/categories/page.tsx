    ...
    >
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {data.categories.map((c: any) => (
          <div key={c.id} className="rounded-2xl border border-cream-300 bg-cream-50 p-4 shadow-sm dark:border-forest-700 dark:bg-forest-800 dark:border-cream-100">
        ...
        {data.categories.length === 0 && (
          <p className="rounded-2xl border border-dashed border-cream-300 p-10 text-center text-sm text-forest-700/60 dark:border-forest-700 dark:text-cream-100/60 dark:border-cream-100">No categories.</p>
        )}
      </div>
    </AdminPage>
  );
