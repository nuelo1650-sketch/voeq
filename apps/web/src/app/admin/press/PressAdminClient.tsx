        ...
        <form onSubmit={submit} className="mt-4 space-y-4">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <label className="text-sm">
              ...
              <select value={kind} onChange={(e) => setKind(e.target.value)} className="w-full rounded-md border border-cream-300 bg-cream-50 px-3 py-2 text-sm dark:border-forest-700 dark:bg-forest-800 dark:border-cream-100">
                {KINDS.map((k) => (
            ...
            <label className="text-sm">
              ...
              <input value={title} onChange={(e) => setTitle(e.target.value)} required className="w-full rounded-md border border-cream-300 bg-cream-50 px-3 py-2 text-sm dark:border-forest-700 dark:bg-forest-800 dark:border-cream-100" />
            </label>
          </div>
          <label className="block text-sm">
            <span className="mb-1 block text-forest-700 dark:text-cream-100">Summary</span>
            <input value={summary} onChange={(e) => setSummary(e.target.value)} className="w-full rounded-md border border-cream-300 bg-cream-50 px-3 py-2 text-sm dark:border-forest-700 dark:bg-forest-800 dark:border-cream-100" />
          </label>
          <label className="block text-sm">
            <span className="mb-1 block text-forest-700 dark:text-cream-100">Body</span>
            <textarea value={body} onChange={(e) => setBody(e.target.value)} rows={5} className="w-full rounded-md border border-cream-300 bg-cream-50 px-3 py-2 text-sm dark:border-forest-700 dark:bg-forest-800 dark:border-cream-100" />
          </label>
          ...
          <div className="flex gap-3">
            ...
            {editingId && (
              <button type="button" onClick={resetForm} className="rounded-md border border-cream-300 px-4 py-2 text-sm text-forest-700 dark:border-forest-700 dark:text-cream-100 dark:border-cream-100">
                Cancel
              </button>
            )}
          </div>
      ...
      <div className="mt-8">
        ...
        {loading ? (
          ...
          <ul className="mt-4 space-y-3">
            {items.map((item) => (
              <li key={item.id} className="flex items-center justify-between rounded-xl border border-cream-300 bg-cream-50 px-4 py-3 dark:border-forest-700 dark:bg-forest-800 dark:border-cream-100">
                <div>
                ...
                <div className="flex gap-2">
                  <button onClick={() => startEdit(item)} className="rounded-md border border-cream-300 px-3 py-1 text-xs text-forest-700 dark:border-forest-700 dark:text-cream-100 dark:border-cream-100">Edit</button>
