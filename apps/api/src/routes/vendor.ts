...
vendorRouter.get('/me/analytics', requireAuth, async (req: AuthedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const [
      ...
      prisma.eventLog.count({ where: { vendorId: vendor.id, eventType: 'conversation_started' } }),
      prisma.eventLog.count({ where: { vendorId: vendor.id, eventType: 'conversation_started', createdAt: { gte: sevenDaysAgo } } }),
    ...
    res.status(200).json({
      stats: {
        ...
        conversationsStartedLast7Days: conversationsLast7Days,
        activeListings,
        totalReviews,
